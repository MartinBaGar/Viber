bl_info = {
    "name": "Pi-pi Interactions Visualizer",
    "author": "Martin Bari Garnier",
    "version": (1, 0),
    "blender": (4, 2, 1),
    "location": "View3D > Sidebar > Pi-Pi Visualizer",
    "description": "Visualizes pi-pi interactions from CSV data",
    "category": "Animation",
}
...
def create_bond_material(interaction_type):
    types = {
        "Parallel stacking": (0.5, 0, 0.5, 1),  # Medium purple
        "T-shaped stacking": (0.6, 0, 0.6, 1),  # Slightly lighter purple
        "Tilted stacking": (0.7, 0, 0.7, 1),    # Even lighter purple
    }
    material_name = f"{interaction_type} Material"
    mat = bpy.data.materials.get(material_name)
    if mat is None:
        mat = bpy.data.materials.new(name=material_name)
        mat.use_nodes = True
        mat.node_tree.nodes["Principled BSDF"].inputs[0].default_value = (
            types[interaction_type][0],
            types[interaction_type][1],
            types[interaction_type][2],
            1,
        )  # Red color
    return mat
