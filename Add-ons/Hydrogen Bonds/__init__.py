...
class OBJECT_OT_hbond_visualizer(Operator):
    ...
    def execute(self, context):
        ...
        if blender_object:
            hbond_props.object_name = blender_object.name  # Store the name of the active object
...
class OBJECT_OT_remake_connection(Operator):
    ...
    def execute(self, context):
        ...
        # Clear previous bond objects
        if "Bonds" in bpy.data.collections:
            bonds_collection = bpy.data.collections["Bonds"]
            for obj in bonds_collection.objects:
                bpy.data.objects.remove(obj, do_unlink=True)
            bpy.data.collections.remove(bonds_collection)
