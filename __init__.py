bl_info = {
    ...
    "description": "Visualizes hydrogen bonds from JSON data",
    "category": "Animation",
}
...
def create_bond_material(interaction_type):
    ...
    if mat is None:
        ...
        if interaction_type == "Hydrogen":
            ...
            principled_bsdf.inputs["Base Color"].default_value = (1, 0.5, 0, 1)  # Orange
            ...
            principled_bsdf.inputs["Base Color"].default_value = (0.5, 0, 0.5, 1)  # Purple
...
def create_bond_objects(all_bonds, bond_material, interaction_type, blender_object=None):
...
def update_bond_positions(scene, depsgraph):
    ...
    object_name = hbond_props.object_name
...
class OBJECT_OT_hbond_visualizer(Operator):
    ...
    def execute(self, context):
        ...
        scene = context.scene
        hbond_props = scene.hbond_visualizer
        json_file = hbond_props.json_file
        
        # Store the active object's name in the scene property
        blender_object = context.active_object
        if blender_object:
            hbond_props.object_name = blender_object.name
        ...
        for interaction_type, frames in frame_dict.items():
            ...
            bond_objects[interaction_type] = create_bond_objects(all_bonds, bond_material, interaction_type)
...
class OBJECT_OT_remake_connection(Operator):
    bl_idname = "object.remake_connection"
    bl_label = "Remake Connection"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        ...
        # Clear existing frame change handlers
        # bpy.app.handlers.frame_change_post.clear()
        
        # Call the hbond visualizer operator
        bpy.ops.object.hbond_visualizer()
        
        self.report({"INFO"}, "H-Bond connection remade")
        return {"FINISHED"}
...
class VIEW3D_PT_hbond_visualizer(Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(hbond_props, "json_file")
        layout.prop(hbond_props, "object_name")
        layout.operator("object.hbond_visualizer")
        layout.operator("object.remake_connection")

classes = (
    HBondVisualizerProperties,
    OBJECT_OT_hbond_visualizer,
    OBJECT_OT_remake_connection,
